...
namespace DataGridExt
{
    ...
    public abstract class DataGridOperator : IDataGridOperator
    {
        ...
        public CellDescriptor CreateCellDescriptor(CellRef cell)
        {
            ...
            d.PropertyDefinition = pd;
            
           // Console.WriteLine(d.Item.GetType().GetProperty("Name").GetValue(d.Item));
            //Console.WriteLine();
            d.Descriptor = this.GetPropertyDescriptor(pd, null, cell);
            d.PropertyType = this.GetPropertyType(cell);
            d.BindingPath = this.GetBindingPath(cell);
            d.BindingSource = this.GetDataContext(cell);
            d.Item = this.GetItem(cell);
            return d;
        }
